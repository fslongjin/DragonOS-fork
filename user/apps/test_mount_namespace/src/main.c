...
int main() {
    ...
    if (pid == 0) {
        ...
        // 挂载tmpfs到测试目录
        printf("   [子进程] 尝试挂载tmpfs到 %s\n", TEST_DIR);
        if (mount("none", TEST_DIR, "tmpfs", 0, NULL) < 0) {
            printf("   [子进程] 错误: mount失败: %s\n", strerror(errno));
            // 如果tmpfs不支持，尝试挂载ramfs
            printf("   [子进程] 尝试挂载ramfs作为备选...\n");
            if (mount("none", TEST_DIR, "ramfs", 0, NULL) < 0) {
                printf("   [子进程] 错误: ramfs mount也失败: %s\n", strerror(errno));
                exit(1);
            }
            printf("   [子进程] ✓ 成功挂载ramfs\n");
        } else {
            printf("   [子进程] ✓ 成功挂载tmpfs\n");
        }
        
        // 在新挂载的文件系统中创建文件
        if (create_test_file(CHILD_FILE, "这是子namespace中新文件系统的文件\n") < 0) {
            exit(1);
        }
        printf("   [子进程] ✓ 在新文件系统中创建测试文件: %s\n", CHILD_FILE);
        ...
        if (mounts) {
            ...
            fclose(mounts);
            if (mount_count > 0) {
                printf("   [子进程] ✓ /proc/mounts可以正常读取\n");
            }
        } else {
            printf("   [子进程] ⚠ 无法读取/proc/mounts: %s\n", strerror(errno));
        }
