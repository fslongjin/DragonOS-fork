...
/// RamFS的inode名称的最大长度
const MYRAMFS_MAX_NAMELEN: usize = 64;
const MYRAMFS_BLOCK_SIZE: u64 = 512;
/// @brief 内存文件系统的Inode结构体
#[derive(Debug)]
struct LockedMyRamFSInode(SpinLock<MyRamFSInode>);
...
pub struct MyRamFSInode {
    ...
    /// 子Inode的B树
    children: BTreeMap<DName, Arc<LockedMyRamFSInode>>,
    /// 当前inode的数据部分
    data: Vec<u8>,
    /// 当前inode的元数据
    metadata: Metadata,
...
impl FileSystem for MyRamFS {
    ...
    fn info(&self) -> FsInfo {
        return FsInfo {
            blk_dev_id: 0,
            max_name_len: MYRAMFS_MAX_NAMELEN,
        };
    }
...
impl MyRamFS {
    pub fn new() -> Arc<Self> {
        ...
        let super_block = SuperBlock::new(
            Magic::MYRAMFS_MAGIC,
            MYRAMFS_BLOCK_SIZE,
            MYRAMFS_MAX_NAMELEN as u64,
        );
        // 初始化root inode
        let root: Arc<LockedMyRamFSInode> = Arc::new(LockedMyRamFSInode(SpinLock::new(MyRamFSInode {
            parent: Weak::default(),
        ...
        let result: Arc<MyRamFS> = Arc::new(MyRamFS {
            root_inode: root,
            super_block: RwLock::new(super_block),
        });

        // 对root inode加锁，并继续完成初始化工作
        let mut root_guard: SpinLockGuard<MyRamFSInode> = result.root_inode.0.lock();
...
static MYRAMFSMAKER: FileSystemMaker = FileSystemMaker::new(
    "myramfs",
    &(MyRamFS::make_ramfs as fn() -> Result<Arc<dyn FileSystem + 'static>, SystemError>),
);

impl IndexNode for LockedMyRamFSInode {
    fn truncate(&self, len: usize) -> Result<(), SystemError> {
        kdebug!("MyRamFS::truncate()");
    ...
    fn read_at(
        ...
        kdebug!("MyRamFS::read_at()");
        if buf.len() < len {
            return Err(SystemError::EINVAL);
        }
        // 加锁
        let inode: SpinLockGuard<MyRamFSInode> = self.0.lock();
    ...
    fn write_at(
        ...
        kdebug!("MyRamFS::write_at()");
        if buf.len() < len {
            return Err(SystemError::EINVAL);
        }

        // 加锁
        let mut inode: SpinLockGuard<MyRamFSInode> = self.0.lock();
    ...
    fn create_with_data(
        ...
        kdebug!("MyRamFS::create_with_data()");
        ...
        // 创建inode
        let result: Arc<LockedMyRamFSInode> = Arc::new(LockedMyRamFSInode(SpinLock::new(MyRamFSInode {
    ...
    fn link(&self, name: &str, other: &Arc<dyn IndexNode>) -> Result<(), SystemError> {
        kdebug!("MyRamFS::link()");
        let other: &LockedMyRamFSInode = other
            .downcast_ref::<LockedMyRamFSInode>()
            .ok_or(SystemError::EPERM)?;
        let name = DName::from(name);
        let mut inode: SpinLockGuard<MyRamFSInode> = self.0.lock();
        let mut other_locked: SpinLockGuard<MyRamFSInode> = other.0.lock();
    ...
    fn move_to(
        ...
        kdebug!("MyRamFS::move_to()");
        let inode_to_move = self
            .find(old_name)?
            .downcast_arc::<LockedMyRamFSInode>()
    ...
    fn mknod(
        ...
        let nod = Arc::new(LockedMyRamFSInode(SpinLock::new(MyRamFSInode {
            parent: inode.self_ref.clone(),
            self_ref: Weak::default(),
            children: BTreeMap::new(),
            data: Vec::new(),
